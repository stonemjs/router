...
describe('UriMatcher', () => {
  describe('matches', () => {
    it('Must return `true` when regex matches', () => {
      // Arrange
      const request = { decodedPathname: '/users/12' }
    ...
    it('Must return `false` when regex not matches', () => {
      // Arrange
      const request = { decodedPathname: 'name.stone' }
    ...
    it('Must return `true` when regex matches alias', () => {
      // Arrange
      const request = { decodedPathname: '/people/12/profile/123/name' }
      const route = {
        pathRegex () {
          return [
    ...
    it('Must return `false` when regex not matches alias', () => {
      // Arrange
      const request = { decodedPathname: '/patate/12/profile/123/name' }
      const route = {
        pathRegex () {
          return [
            /^\/users\/(\d+)\/profile\/([^/]+?)\/name$/,
